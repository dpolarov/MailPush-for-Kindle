...
COPY mailpush.koplugin/_meta.lua mailpush.koplugin/main.lua mailpush.koplugin/updater.lua mailpush.koplugin/updater_download.lua mailpush.koplugin/config.default.json /work/mailpush.koplugin/
RUN printf '%s\n' "$VERSION" > /work/mailpush.koplugin/VERSION && \
    cp /etc/ssl/certs/ca-certificates.crt /work/mailpush.koplugin/cacert.pem && \
    chmod 755 /work/mailpush.koplugin/bin/mailpush && \
    mkdir -p /dist && \

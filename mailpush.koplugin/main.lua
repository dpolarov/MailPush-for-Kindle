...
local MailPush = WidgetContainer:extend{
    name = "mailpush",
    is_doc_only = false,
}
...
local function load_config()
    ...
    if not raw then return nil, "Cannot read configuration file." end
...
function MailPush:run_backend(command, quiet)
    if not read_all(binary_path) then
        self:show("MailPush backend is missing. Rebuild the project and reinstall the plugin.", true)
        return nil
    end
    ...
    if not quiet then self:show(command == "test" and "Testing IMAP connection…" or "Checking mail…") end
...
local function hint_for(text)
    ...
    if s:find("certificate", 1, true) or s:find("x509", 1, true) then return "Hint: Check the Kindle date/time and the CA settings. Public services should work with the bundled CA set." end
    if s:find("cannot connect", 1, true) or s:find("connection refused", 1, true) then return "Hint: Check Wi-Fi, IMAP host, port, and whether IMAP access is enabled for the account." end
    if s:find("timeout", 1, true) or s:find("deadline exceeded", 1, true) then return "Hint: Check Wi-Fi quality and the server address. You can increase timeouts in config.json if needed." end
    if s:find("configuration", 1, true) or s:find("imap host is empty", 1, true) then return "Hint: Open MailPush settings and fill in the IMAP connection fields." end
    if s:find("outside configured root", 1, true) or s:find("symbolic link", 1, true) then return "Hint: Change the save path so it stays inside Allowed root directory." end
    if s:find("size limit", 1, true) or s:find("too many files", 1, true) then return "Hint: The safety limit blocked this item. Increase the corresponding limit only if you trust the message." end
    if s:find("archive", 1, true) then return "Hint: The archive may be damaged or contain an unsafe entry. Disable automatic unpacking to keep the original archive." end
    return nil
end
...
function MailPush:show_result(result)
    ...
    if #files > 0 then
        ...
        local max_show = math.min(#files, 12)
        for i = 1, max_show do table.insert(lines, "• " .. basename(files[i])) end
        if #files > max_show then table.insert(lines, string.format("…and %d more.", #files - max_show)) end
    end
    local errors = result.errors or {}
    if #errors > 0 then
        table.insert(lines, "")
        table.insert(lines, "Errors:")
        local max_errors = math.min(#errors, 8)
        for i = 1, max_errors do table.insert(lines, "• " .. tostring(errors[i])) end
        if #errors > max_errors then table.insert(lines, string.format("…and %d more errors.", #errors - max_errors)) end
    end
    if not result.ok then
        local hint = hint_for((result.message or "") .. " " .. table.concat(errors, " "))
        if hint then table.insert(lines, ""); table.insert(lines, hint) end
    end
...
function MailPush:fetch(quiet)
    ...
    local result = self:run_backend("fetch", quiet)
    self:show_result(result)
end

function MailPush:test_connection()
    local result = self:run_backend("test", false)
    self:show_result(result)
end
...
function MailPush:reset_history()
    UIManager:show(ConfirmBox:new{
        text = "Forget the processed-message history?\n\nPreviously downloaded messages may be downloaded again on the next check.",
        ok_text = "Reset",
